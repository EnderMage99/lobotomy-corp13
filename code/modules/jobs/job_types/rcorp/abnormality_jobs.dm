...
/proc/initialize_abnormality_job_positions()
	...
	for(var/datum/job/rcorp_abnormality/J in SSjob.occupations)
		if(!J.landmark_type)
			continue
		var/list/landmarks = J.get_landmark_list()
		if(landmarks)
...
/datum/job/rcorp_abnormality
	...
	maptype = list("rcorp", "rcorp_fifth")
...
/datum/job/rcorp_abnormality/proc/get_landmark_list()
	switch(landmark_type)
		if("easycombat")
			return GLOB.landmark_positions_easycombat
		if("easysupport")
			return GLOB.landmark_positions_easysupport
		if("easytank")
			return GLOB.landmark_positions_easytank
		if("hardcombat")
			return GLOB.landmark_positions_hardcombat
		if("hardsupport")
			return GLOB.landmark_positions_hardsupport
		if("hardtank")
			return GLOB.landmark_positions_hardtank
		if("rhinobuster")
			return GLOB.landmark_positions_rhinobuster
		if("raidboss")
			return GLOB.landmark_positions_raidboss
	return null

/datum/job/rcorp_abnormality/proc/get_abno_pool()
	switch(abno_list)
		if("easycombat")
			return GLOB.easycombat
		if("easysupport")
			return GLOB.easysupport
		if("easytank")
			return GLOB.easytank
		if("hardcombat")
			return GLOB.hardcombat
		if("hardsupport")
			return GLOB.hardsupport
		if("hardtank")
			return GLOB.hardtank
		if("rhinobuster")
			return GLOB.rhinobuster
		if("raidboss")
			return GLOB.raidboss
	return null
...
/datum/job/rcorp_abnormality/New()
	...
	if(landmark_type)
		var/list/landmarks = get_landmark_list()
		if(landmarks)
...
/datum/job/rcorp_abnormality/after_spawn(mob/living/carbon/human/H, mob/M, latejoin = FALSE)
	...
	var/list/available_landmarks = get_landmark_list()
	if(!available_landmarks || !length(available_landmarks))
	...
	var/list/abno_pool = get_abno_pool()
	if(!abno_pool || !length(abno_pool))
